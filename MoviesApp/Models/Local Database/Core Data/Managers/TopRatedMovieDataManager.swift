//
//  TopRatedMovieDataManager.swift
//  MoviesApp
//
...
class TopRatedMovieDataManager: MovieDataInterface {

  let dbHelper = CoreDataHelper.shared
  typealias ModelType = TopRatedMovieCD
  
  func saveMovies(_ movies: [MovieModel]) {
    movies.forEach { item in
      let object = TopRatedMovieCD(context: dbHelper.context)
      object.id = "\(item.id)"
